    ...
    <UniversalPageLayout>
      ...
      <KPIGrid>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-bold text-foreground">{filteredHistoryCount}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Companies House Reports</p>
          <p className="mt-2 text-3xl font-bold text-foreground">{companiesHouseCount}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Preview Status</p>
          <p className="mt-2 text-3xl font-bold text-foreground">{previewHtml ? 'Ready' : 'Waiting'}</p>
        </div>
      </KPIGrid>
      ...
      <ContentSection>
        <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <div className="flex flex-col gap-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Builder</p>
        ...
        <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <div className="flex flex-col gap-1">
            ...
            <h2 className="text-xl font-semibold text-foreground">Live Report Preview</h2>
          </div>
          {previewHtml ? (
            <iframe
              title="Report preview"
              srcDoc={previewHtml}
              className="mt-4 min-h-[700px] w-full rounded-xl border border-border bg-card"
            />
          ) : (
            <p className="mt-4 text-sm text-muted-foreground">Generate a report to preview it here.</p>
          )}
        </section>
        ...
        <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="mt-6 overflow-x-auto">
            {isLoading ? (
              <p className="py-6 text-sm text-muted-foreground">Loading reports…</p>
            ) : history.length === 0 ? (
              <p className="py-6 text-sm text-muted-foreground">No saved reports yet.</p>
            ) : (
              <table className="min-w-full border-collapse text-sm">
                <thead>
                  <tr className="border-b border-border text-left">
                    <th className="px-3 py-3 font-semibold text-muted-foreground">Title</th>
                    <th className="px-3 py-3 font-semibold text-muted-foreground">Type</th>
                    <th className="px-3 py-3 font-semibold text-muted-foreground">Generated</th>
                    <th className="px-3 py-3 font-semibold text-muted-foreground">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map((report) => (
                    <tr key={report.report_id} className="border-b border-border align-top">
                      <td className="px-3 py-4 text-foreground">{report.title}</td>
                      <td className="px-3 py-4 text-muted-foreground">{report.report_type}</td>
