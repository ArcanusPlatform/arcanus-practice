    ...
    <UniversalPageLayout>
          ...
          <div className="rounded-xl border border-border bg-muted p-1">
            <button
              className={`rounded-lg px-4 py-2 text-sm font-semibold ${
                viewMode === 'engagements' ? 'bg-primary text-primary-foreground' : 'text-foreground'
              }`}
            ...
            <button
              className={`ml-2 rounded-lg px-4 py-2 text-sm font-semibold ${
                viewMode === 'templates' ? 'bg-primary text-primary-foreground' : 'text-foreground'
              }`}
          ...
          {viewMode === 'engagements' ? (
            <>
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Engagements</p>
                  <h2 className="text-xl font-semibold text-foreground">Client Agreed Services</h2>
                  <p className="text-sm text-muted-foreground">Annualized value: {currency.format(annualizedServiceRevenue)}</p>
                </div>
              ...
              <div className="mt-4" style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={10} style={{ padding: '1rem', color: 'var(--color-text-muted)' }}>
                      ...
                      filteredServices.map((service) => (
                        <tr key={service.id} style={{ borderTop: '1px solid var(--color-border)' }}>
                          <td style={{ padding: '0.75rem', fontWeight: 600 }}>
                            <p style={{ margin: 0, fontFamily: 'monospace', fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>
                              {service.clientRef}
                            </p>
                          ...
                          <td style={{ padding: '0.75rem' }}>
                            ...
                            <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>{service.serviceCode}</p>
                          </td>
            ...
            <>
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Template Library</p>
                  <h2 className="text-xl font-semibold text-foreground">Service Templates</h2>
                </div>
                  ...
                  Object.entries(templateSections).map(([category, rows]) => (
                    <div key={category} className="rounded-xl border border-border bg-card">
                      <div className="border-b border-border px-4 py-3">
                        ...
                        <h3 className="text-base font-semibold text-foreground">{rows.length} templates</h3>
                      </div>
                      <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          ...
                          <tbody>
                            {rows.map((template) => (
                              <tr key={template.serviceCode} style={{ borderTop: '1px solid var(--color-border)' }}>
                                <td style={{ padding: '0.75rem' }}>
      ...
      {selectedTemplate && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 p-4">
          <div className="mx-auto mt-8 w-full max-w-3xl rounded-2xl border border-border bg-card shadow-2xl">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Service Template</p>
            ...
            <div className="grid gap-4 px-6 py-5 md:grid-cols-2">
              ...
              <div className="rounded-xl border border-border bg-muted p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Client Type Mapping</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {selectedTemplate.clientTypes.map((type) => (
                    <span key={type} className="rounded-full bg-card px-3 py-1 text-xs font-semibold text-foreground">
                      {type}
                    </span>
                  ))}
                </div>
                <p className="mt-3 text-sm text-foreground">{selectedTemplate.description || 'No description.'}</p>
              </div>
            </div>

            <div className="border-t border-border px-6 py-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Task Template Steps</p>
                  ...
                  <div key={task.id} className="rounded-lg border border-border p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
                        ...
                        <span className="rounded-full bg-muted px-2 py-1">{task.daysBeforeDue} days before due</span>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {isEditorOpen && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 p-4">
          <div className="mx-auto mt-8 w-full max-w-4xl rounded-2xl border border-border bg-card shadow-2xl">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <h3 className="text-xl font-semibold text-foreground">
                {editorMode === 'create' ? 'Add Service Template' : `Edit Template ${templateDraft.serviceCode}`}
              </h3>
              <button className="rounded-lg border border-border p-2 text-muted-foreground" onClick={() => setIsEditorOpen(false)}>
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-4 px-6 py-5">
              ...
              <div className="rounded-xl border border-border p-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-foreground">Task Templates</p>
                  <button
                    className="btn-secondary"
                    onClick={() =>
                      setTemplateDraft((prev) => ({
                ...
                <div className="mt-3 space-y-2">
                  {templateDraft.taskTemplates.map((task, index) => (
                    <div key={task.id} className="grid gap-2 rounded-lg border border-border p-3 md:grid-cols-[2fr,1fr,1fr,auto]">
                      <input
                        className="rounded-lg border border-border px-3 py-2 text-sm"
              ...
              <div className="rounded-lg border border-border p-4">
                <p className="text-sm font-semibold text-foreground">Client Type Mapping</p>
