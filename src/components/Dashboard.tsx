      ...
      <UniversalPageHeader
        ...
        meta={
          <div className="space-y-1">
            <p>
              <span className="font-semibold text-foreground">{snapshot.overdueTasks}</span> overdue tasks and{' '}
              <span className="font-semibold text-foreground">{snapshot.overdueCompliance}</span> overdue compliance lines.
            </p>
      ...
      <ContentSection>
        <section className="grid gap-6 xl:grid-cols-[1.15fr,0.85fr]">
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Next 7 Days</p>
            ...
            {isLoading ? (
              ...
              <div className="mt-8 rounded-2xl border border-dashed border-border bg-muted px-6 py-10 text-center">
                ...
                <p className="mt-3 text-sm text-muted-foreground">No tasks due in the next 7 days.</p>
              </div>
            ) : (
              <div className="mt-6 space-y-3">
                {snapshot.upcomingTasks.map((task) => (
                  <button
                    ...
                    className="grid w-full gap-3 rounded-2xl border border-border px-4 py-4 text-left transition hover:border-cyan-200 hover:bg-muted md:grid-cols-[minmax(0,1fr)_auto]"
                  >
                    <div>
                    ...
                    <div className="flex flex-col items-start gap-2 md:items-end">
                      ...
                      <p className="text-sm font-semibold text-foreground">{formatDueDate(task.dueDate)}</p>
                      <p className="text-xs text-muted-foreground">{timingLabel(dayDiffFromToday(task.dueDate))}</p>
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Action Queue</p>
                <h2 className="text-xl font-semibold text-foreground">Late and Due Compliance</h2>
              </div>
            ...
            {isLoading ? (
              <p className="mt-6 text-sm text-muted-foreground">Loading compliance items…</p>
            ) : snapshot.urgentCompliance.length === 0 ? (
              <div className="mt-8 rounded-2xl border border-dashed border-border bg-muted px-6 py-10 text-center">
                <CheckCircle2 className="mx-auto h-10 w-10 text-muted-foreground" />
                <p className="mt-3 text-sm text-muted-foreground">No late or near-term compliance items.</p>
              </div>
            ) : (
              <div className="mt-6 space-y-3">
                {snapshot.urgentCompliance.map((item) => (
                  <button
                    ...
                    className="w-full rounded-2xl border border-border px-4 py-4 text-left transition hover:border-cyan-200 hover:bg-muted"
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <p className="text-sm font-semibold text-foreground">{formatDueDate(item.dueDate)}</p>
                      <p className="text-xs text-muted-foreground">{timingLabel(dayDiffFromToday(item.dueDate))}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.35fr,0.95fr]">
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Workload Outlook</p>
                <h2 className="text-xl font-semibold text-foreground">Task Load Over 6 Weeks</h2>
              </div>
              <CalendarClock className="h-5 w-5 text-muted-foreground" />
            </div>

            {isLoading ? (
              <p className="mt-8 text-sm text-muted-foreground">Loading workload trend…</p>
            ) : (
              <>
                <div className="mt-6 grid grid-cols-6 gap-3">
                  {snapshot.workloadBuckets.map((bucket) => (
                    <div key={bucket.label} className="flex flex-col items-center gap-3">
                      <div className="flex h-44 w-full items-end rounded-2xl bg-muted px-2 pb-2">
                        <div
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                  <div className="rounded-xl border border-border bg-muted px-4 py-4">
                    ...
                    <p className="mt-2 text-2xl font-bold text-foreground">{snapshot.taskCompletionRate}%</p>
                  </div>
                  <div className="rounded-xl border border-border bg-muted px-4 py-4">
                  ...
                  <div className="rounded-xl border border-border bg-muted px-4 py-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Due This Week</p>
                    <p className="mt-2 text-2xl font-bold text-foreground">{snapshot.tasksNext7}</p>
                  </div>
                </div>
              </>
            )}
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Compliance Radar</p>
                <h2 className="text-xl font-semibold text-foreground">Late and Upcoming Compliance</h2>
              </div>
              <AlertTriangle className="h-5 w-5 text-muted-foreground" />
            </div>

            {isLoading ? (
              <p className="mt-8 text-sm text-muted-foreground">Loading compliance profile…</p>
            ) : (
