    ...
    return (
      <div className="p-10">
        <div className="mx-auto max-w-3xl rounded-2xl border border-border bg-card p-10 text-center shadow-sm">
          ...
          <h1 className="text-2xl font-semibold text-foreground">Client Workspace</h1>
          <p className="mt-4 text-muted-foreground">
            Client management is available for Agent accounts with multi-client workflows.
          </p>
          <button
            onClick={() => navigate('/settings')}
            className="mt-6 rounded-lg bg-primary px-6 py-3 font-semibold text-primary-foreground hover:bg-primary/90"
          >
            Review Settings
          </button>
        </div>
      ...
      <UniversalPageHeader
        ...
        actions={
          <>
            ...
            />
            <button
              className="px-4 py-2 border border-border rounded-lg hover:bg-muted transition-colors flex items-center gap-2 font-semibold text-foreground disabled:opacity-60"
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 flex items-end justify-between">
              <span className="text-3xl font-semibold text-foreground">{summary.total}</span>
              <CheckCircle2 className="h-8 w-8 text-emerald-500" />
            </div>
            <p className="mt-2 text-xs text-muted-foreground">From client database records</p>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Missing Onboarding Data</p>
            <div className="mt-4 flex items-end justify-between">
              <span className="text-3xl font-semibold text-foreground">{summary.missingOnboarding}</span>
              <AlertTriangle className="h-8 w-8 text-rose-500" />
            </div>
            <p className="mt-2 text-xs text-muted-foreground">EORI is optional and does not block onboarding</p>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Clients With Services</p>
            <div className="mt-4 flex items-end justify-between">
              <span className="text-3xl font-semibold text-foreground">{summary.withServices}</span>
              <Wrench className="h-8 w-8 text-amber-500" />
            </div>
            <p className="mt-2 text-xs text-muted-foreground">Agreed service engagements</p>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Annual Service Cost</p>
            <div className="mt-4 flex items-end justify-between">
            ...
            <p className="mt-2 text-xs text-muted-foreground">Sum of active client services</p>
          </div>
        </div>
        ...
        <div className="mt-10">
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex flex-1 items-center gap-3 rounded-xl border border-border px-3 py-2">
                ...
                <input
                  ...
                  className="h-8 w-full border-0 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none"
                />
              </div>
              ...
              <div className="w-full md:w-auto">
                <select
                  ...
                  className="h-10 min-w-[140px] rounded-xl border border-border bg-background px-3 text-sm text-foreground"
                >
                ...
                {[
                  ...
                  <button
                    ...
                    className={`rounded-full px-4 py-1 ${
                      focusFilter === filter.value ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                    }`}
                  >
                    {filter.label}
                  </button>
            ...
            <div className="mt-5 overflow-x-auto">
              {isLoading ? (
                <p className="py-10 text-center text-sm text-muted-foreground">Loading clients...</p>
              ) : error ? (
                <p className="py-10 text-center text-sm text-rose-500">{error}</p>
              ) : filteredRows.length === 0 ? (
                <p className="py-10 text-center text-sm text-muted-foreground">No clients match the current filters.</p>
              ) : (
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Main Contact</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Phone</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Year End</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Accounts Due</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">CS Due</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-muted-foreground">Fees</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-muted-foreground">Tasks Due</th>
                    ...
                    {paginatedRows.map((row) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <span className="font-mono text-sm font-semibold text-foreground">{row.clientRef}</span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <p className="text-sm font-semibold text-foreground">{row.mainContact}</p>
                          <p className="mt-1 text-xs text-muted-foreground">{row.mainContactDetail}</p>
                        </td>
                        <td className="px-4 py-3 text-sm text-muted-foreground">{row.phone}</td>
                        <td className="px-4 py-3 text-sm text-muted-foreground">{row.yearEnd}</td>
                        <td className="px-4 py-3 text-sm text-muted-foreground">{row.accountsDue}</td>
                        <td className="px-4 py-3 text-sm text-muted-foreground">{row.confirmationStatementDue}</td>
                        <td className="px-4 py-3 text-right text-sm font-semibold text-foreground">
                          {currencyFormatter.format(row.annualServiceCost)}
                        </td>
                        <td className="px-4 py-3 text-right text-sm font-semibold text-foreground">{row.openTasks}</td>
                        <td className="px-4 py-3 text-right">
                          <button
                            type="button"
                            className="rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-foreground transition hover:border-border hover:bg-muted"
                            onClick={(event) => {
            ...
            {!isLoading && !error && filteredRows.length > 0 && (
              <div className="mt-4 flex flex-col gap-3 border-t border-border pt-4 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
                <p>
                  ...
                  of <span className="font-semibold text-foreground">{filteredRows.length}</span> clients
                </p>
                {pageSize !== 'all' && totalPages > 1 && (
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      className="rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-foreground transition hover:border-border hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
