...
import { FieldRef, Value } from '@prisma/client/runtime/client';
import { AccountsSetStatus } from '@/types/accounts-production';
...
type FieldStatus = NonNullable<Contact['field_statuses']>[string];

function getEffectiveFieldStatus(
  contact: Contact,
  fieldKey: string
    ...
    return (
      <dl className="text-sm text-muted-foreground">
        ...
        <dd className="flex items-center gap-2">
          {hasValue ? <span>{value}</span> : <span className="text-muted-foreground">Awaiting registration</span>}
  ...
  if (status === 'not_applicable') {
    return (
      <dl className="text-sm text-muted-foreground">
        <dt className="font-semibold text-foreground">{label}</dt>
        <dd>
          <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">Not applicable</span>
        </dd>
      </dl>
    );
  }
...
export default function ClientDetail() {
  ...
  const onboardingChecks = useMemo(() => {
    ...
    const statusApplied = (field: string) =>(fieldStatuses[field]);
  ...
  const lastAccountsMadeUpTo = formatDate(
    (accountsCompliance?.next_made_up_to || accountsCompliance?.last_accounts?.made_up_to || accountsCompliance?.last_accounts?.period_end_on) as
      | string
      | undefined
  );
...
const updateFieldStatusDraft = (
  ...
  if (userType === 'SELF') {
    return (
      <div className="p-8">
        <div className="mx-auto max-w-3xl rounded-2xl border border-border bg-card0 text-center shadow-sm">
          <h1 className="text-2xl font-semibold text-foreground">Importer Workspace Restricted</h1>
  ...
  return (
    <div className="bg-muted">
      <div className="mx-auto max">
        ...
        {loading ? (
          <div className="mt-12 rounded-2xl border border-border bg-card0 text-center text-muted-foreground shadow-sm">
          ...
          <>
            <section className="mt-6 rounded-3xl border border-border bg-card p-8 shadow-sm">
                ...
                <div>
                  ...
                  <div className="mt-3 space-y-1">
                    <p className="text-sm font-semibold text-foreground">
                      Company Number <span className="font-normal text-muted-foreground">{profileCompanyNumber}</span>
                    </p>
                    <p className="text-sm text-muted-foreground">{profileRegisteredOffice}</p>
                  </div>
                </div>

                <div className="flex min-w-[240px] flex-col items-end justify-between gap-3 md:self-stretch">
                  <div className="text-right">
                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Client Ref</p>
            ...
            <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              >
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Annual Fees</p>
                  <Wallet className="h-4 w-4 text-emerald-600" />
                </div>
                <p className="mt-2 text-2xl font-bold text-foreground">{currencyFormatter.format(annualFeeValue)}</p>
                <p className="text-xs text-muted-foreground">{currencyFormatter.format(recurringMonthly)} monthly recurring</p>
              </article>
              <article className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                ...
                <p className="mt-2 text-2xl font-bold text-foreground">{taskSummary.open}</p>
              ...
              <article className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Compliance Status</p>
                  <ShieldCheck className="h-4 w-4 text-blue-600" />
                </div>
                <p className="mt-2 text-2xl font-bold text-foreground">{complianceCoverageRate}%</p>
                <p className="text-xs text-muted-foreground">{complianceServices.length} covered services</p>
                <div className="mt-2 h-1.5 rounded-full bg-muted">
                <div
              ...
              <article className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Next Filing Due</p>
                  <FileText className="h-4 w-4 text-violet-600" />
                </div>
                <p className="mt-2 text-xl font-bold text-foreground">{nextComplianceDate}</p>
            ...
            {activeTab === 'overview' && (
              <>
                <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_340px]">
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                          Operational Snapshot
                        </p>
                        <h2 className="text-xl font-semibold text-foreground">Practice Overview</h2>
                      </div>
                      <Sparkles className="h-4 w-4 text-amber-500" />
                    </header>
                    <div className="mt-4 grid gap-4 md:grid-cols-3">
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Onboarding Completion</p>
                        <p className="mt-1 text-2xl font-bold text-foreground">{onboardingChecks.completionRate}%</p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Compliance Rate</p>
                        <p className="mt-1 text-2xl font-bold text-foreground">{complianceCoverageRate}%</p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Task Completion</p>
                        <p className="mt-1 text-2xl font-bold text-foreground">{taskCompletionRate}%</p>
                      </div>
                    </div>
                    <div className="mt-5 rounded-xl border border-border/60 bg-muted">
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Trend Signal</p>
                    ...
                    <div className="mt-4 grid gap-2 text-xs text-muted-foreground md:grid-cols-2">
                      {onboardingChecks.checks.map((check) => (
                        <div key={check.label} className="flex items-center justify-between rounded-lg border border-border/60 bg-card px-3 py-2">
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header>
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Smart Action</p>
                      <h2 className="text-xl font-semibold text-foreground">Priority Queues</h2>
                    </header>
                    <div className="mt-4 space-y-3">
                      <div className="rounded-xl border border-rose-200 bg-card">
                        <p className="text-xs font-semibold uppercase tracking-wide text-rose-700">Critical</p>
                        {smartActions.critical.length === 0 ? (
                          <p className="mt-2 text-sm text-muted-foreground">No blocking issues.</p>
                        ) : (
                      ...
                      <div className="rounded-xl border border-amber-200 bg-card">
                        <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Risk</p>
                        {smartActions.risk.length === 0 ? (
                          <p className="mt-2 text-sm text-muted-foreground">No immediate exposure.</p>
                        ) : (
                          <ul className="mt-2 space-y-2 text-sm text-foreground">
                      ...
                      <div className="rounded-xl border border-amber-200 bg-card">
                        <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Advisory</p>
                        {smartActions.advisory.length === 0 ? (
                          <p className="mt-2 text-sm text-muted-foreground">No advisory actions pending.</p>
                        ) : (
                          <ul className="mt-2 space-y-2 text-sm text-foreground">
                ...
                <section className="mt-6 grid gap-6 xl:grid-cols-2">
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-center justify-between">
                      <div>
                        ...
                        <h2 className="text-xl font-semibold text-foreground">{profileCompanyName}</h2>
                      </div>
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header>
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Financial Snapshot</p>
                      <h2 className="text-xl font-semibold text-foreground">Commercial Position</h2>
                    </header>
                    <div className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Annual Fee</p>
                        <p className="text-xl font-bold text-foreground">{currencyFormatter.format(annualFeeValue)}</p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Monthly Recurring</p>
                        <p className="text-xl font-bold text-foreground">{currencyFormatter.format(recurringMonthly)}</p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Outstanding Invoices</p>
                        <p className={`text-xl font-bold ${outstandingInvoices > 0 ? 'text-rose-700' : 'text-foreground'}`}>
                          {outstandingInvoices}
                        </p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Realised Revenue</p>
                        <p className="text-xl font-bold text-foreground">{currencyFormatter.format(revenueYtd)}</p>
                      </div>
                    </div>
                    <div className="mt-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-center justify-between">
                      <div>
                        ...
                        <h2 className="text-xl font-semibold text-foreground">Client Service Portfolio</h2>
                    ...
                    <div className="mt-4 space-y-4">
                      {clientServices.length === 0 ? (
                        <p className="text-sm text-muted-foreground">No services have been agreed for this client yet.</p>
                      ) : (
                        clientServices.map((service) => {
                          ...
                          return (
                            <article key={service.id} className="rounded-2xl border border-border bg-muted/80 p-5 shadow-sm">
                                ...
                                <div>
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <span
                                      className={`rounded-full px-3 py-1 text-xs font-semibold ${
                                        service.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'
                                      }`}
                                    >
                                  ...
                                  <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                                    ...
                                    <div className="rounded-xl border border-border bg-card">
                                      ...
                                      <p className="mt-1 text-sm font-semibold text-foreground">{formatDate(service.nextDue)}</p>
                                    </div>
                                    <div className="rounded-xl border border-border bg-card">
                                      <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                                        {scheduleRule.inputLabel}
                                      </p>
                                      <p className="mt-1 text-sm font-semibold text-foreground">{formatDate(scheduleAnchorDate)}</p>
                                    </div>
                                ...
                                <div className="mt-5 border-t border-border pt-5">
                                  <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]">
                                    <div className="space-y-4">
                                      ...
                                      <div className="rounded-xl border border-border bg-card">
                                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Scheduling Notes</p>
                                        <p className="mt-2 text-sm text-muted-foreground">{scheduleRule.helperText}</p>
                                      </div>
                                    </div>

                                    <div className="space-y-4">
                                      <div className="rounded-xl border border-border bg-card">
                                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Service Snapshot</p>
                                          ...
                                          <div className="mt-3 space-y-2">
                                            {service.complianceDates.map((deadline) => (
                                              <div
                                                ...
                                                className="rounded-lg border border-border bg-muted px-3 py-3 text-sm text-foreground"
                                              >
                                                <p className="font-semibold text-foreground">{deadline.label}</p>
                                                <p className="mt-1 text-muted-foreground">Due {formatDate(deadline.dueDate)}</p>
                                              </div>
                                            ))}
                                          </div>
                                        </div>
                                  ...
                                  <div className="mt-5 rounded-xl border border-border bg-card">
                                    ...
                                    {service.taskInstances.length === 0 ? (
                                      <p className="mt-3 text-sm text-muted-foreground">No tasks linked to this service.</p>
                                    ) : (
                                      <div className="mt-3 space-y-3">
                                        {service.taskInstances.map((task) => (
                                          <div
                                            key={task.id}
                                            className="grid gap-3 rounded-xl border border-border bg-muted md:grid-cols-[minmax(0,1.7fr)_140px_170px]"
                                          >
                                            <div>
                                            ...
                                            <div className="flex items-start md:justify-center">
                                              <span className="inline-flex h-fit rounded-full bg-muted px-3 py-1 text-xs font-semibold text-foreground">
                                                {task.priority}
                                              </span>
                                            </div>
                                            <select
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-center justify-between">
                      <div>
                        ...
                        <h2 className="text-xl font-semibold text-foreground">Service Task Progress</h2>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 text-sm">
                      <div className="rounded-xl border border-border/60 bg-muted">
                        ...
                        <p className="text-2xl font-bold text-foreground">{taskSummary.open}</p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                      ...
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Compliance services</p>
                        <p className="text-2xl font-bold text-foreground">{complianceServices.length}</p>
                      </div>
                      <div className="rounded-xl border border-border/60 bg-muted">
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Annual service value</p>
                ...
                <section className="mt-6 grid gap-6 xl:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Services</p>
                        <h2 className="text-xl font-semibold text-foreground">Recent Task Activity</h2>
                      </div>
                      <FileText className="h-5 w-5 text-blue-500" />
                    </header>
                    <div className="mt-4 space-y-3">
                      {clientServices.length === 0 ? (
                        <p className="text-sm text-muted-foreground">No service activity recorded for this client.</p>
                      ) : (
                        clientServices
                          .flatMap((service) =>
                            service.taskInstances.map((task) => ({
                          ...
                          <article
                            key={taskItem.id}
                            className="rounded-xl border border-border/60 bg-muted"
                          >
                            <div className="flex items-center justify-between text-sm">
                              <div>
                                <p className="font-semibold text-foreground">{taskItem.title}</p>
                                <p className="text-xs text-muted-foreground">{taskItem.service}</p>
                              </div>
                              <div className="text-right">
                                <span
            ...
            {activeTab === 'compliance' && (
              <section className="mt-6 grid gap-6 xl:grid-cols-2">
                <div className="rounded-2xl border border-border bg-cardshadow-sm">
                  <header className="flex items-center justify-between">
                    <div>
                      ...
                      <h2 className="text-xl font-semibold text-foreground">Upcoming Deadlines (Next 90 Days)</h2>
                    </div>
                  ...
                  <div className="mt-4 space-y-3">
                    {complianceDeadlineSummary.upcoming.length === 0 ? (
                      ...
                      complianceDeadlineSummary.upcoming.map((item) => (
                        <article key={item.id} className="rounded-xl border border-border/60 bg-muted">
                          <div className="flex items-center justify-between">
                          ...
                          <p className="mt-1 text-sm text-muted-foreground">Due {item.dueDate.toLocaleDateString('en-GB')}</p>
                        </article>
                      ))
                    )}
                  </div>
                ...
                <div className="rounded-2xl border border-border bg-cardshadow-sm">
                  <header>
                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Regulatory Controls</p>
                    <h2 className="text-xl font-semibold text-foreground">Compliance Panel</h2>
                  </header>
                  ...
                  <div className="mt-4 rounded-xl border border-border/60 bg-muted text-sm text-foreground">
              ...
              <>
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <h2 className="text-xl font-semibold text-foreground">Engagement & Authority</h2>
                      </div>
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <h2 className="text-xl font-semibold text-foreground">Usernames and Codes</h2>
                      </div>
                      <button className="btn-secondary" onClick={() => openSectionEditor('engagement')}>
                        Edit
                      </button>
                ...
                <section className="mt-6 grid gap-6 xl:grid-cols-2">
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">AML & Risk</p>
                        <h2 className="text-xl font-semibold text-foreground">Risk & Verification</h2>
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="rounded-2xl border border-border bg-cardshadow-sm">
                    <header className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">HMRC & Tax</p>
                        <h2 className="text-xl font-semibold text-foreground">Tax Registrations</h2>
                      </div>
                ...
                <section className="mt-6 rounded-2xl border border-border bg-cardshadow-sm">
                  <header className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                        Internal Practice Structure
                      </p>
                      <h2 className="text-xl font-semibold text-foreground">Ownership & Operations</h2>
                    </div>
              ...
              <section className="mt-6 rounded-2xl border border-border bg-cardshadow-sm">
                <header className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="text-xl font-semibold text-foreground">Embedded Company Profile</h2>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                ...
                {companiesHouseTab === 'company-details' && (
                  <div className="mt-6 grid gap-4 xl:grid-cols-3">
                    <section className="rounded-2xl border border-border bg-muted p-5 shadow-sm">
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Company Profile</p>
                ...
                {companiesHouseTab === 'officers' && (
                  <div className="mt-6 space-y-3">
                    {activeOfficers.length ? (
                      <>
                        <div className="hidden rounded-xl border border-border bg-muted px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground md:grid md:grid-cols-4">
                          <span>Name</span>
                          <span>Role</span>
                          ...
                          return (
                            ...
                            >
                              <div className="grid gap-3 md:grid-cols-4 md:items-start">
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Name</p>
                                  ...
                                  <div className="mt-2 flex flex-wrap items-center gap-2">
                                    {linkedClient ? (
                                      <>
                                        ...
                                        <button
                                          type="button"
                                          className="rounded-full border border-border px-3 py-1 text-xs font-semibold text-foreground transition hover:border-border hover:bg-card"
                                      ...
                                      <button
                                        type="button"
                                        className="rounded-full border border-border px-3 py-1 text-xs font-semibold text-foreground transition hover:border-border hover:bg-card disabled:cursor-not-allowed disabled:opacity-60"
                                ...
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Role</p>
                                  <p className="text-sm text-foreground">{formatLabel(officer.role)}</p>
                                </div>
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Appointment</p>
                                  <p className="text-sm text-foreground">{formatDate(officer.appointedOn)}</p>
                                </div>
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Resigned</p>
                      ...
                      <p className="text-sm text-muted-foreground">No current officers returned in this snapshot.</p>
                    )}
                              ...
                              return (
                                ...
                                >
                                  <div className="grid gap-3 md:grid-cols-4 md:items-start">
                                    ...
                                    <div>
                                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Resigned</p>
                                      <p className="text-sm text-foreground">{formatDate(officer.resignedOn)}</p>
                                    </div>
                                  </div>
                                </article>
                              );
                ...
                {companiesHouseTab === 'psc' && (
                  <div className="mt-6 space-y-3">
                    {activePscEntries.length ? (
                          ...
                          return (
                            ...
                            >
                              <div className="grid gap-3 md:grid-cols-4 md:items-start">
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Name</p>
                                  <p className="font-semibold text-foreground">{psc.name}</p>
                                  <div className="mt-2 flex flex-wrap items-center gap-2">
                                    {linkedClient ? (
                                      <>
                                        <span className="text-xs font-medium text-muted-foreground">
                                ...
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Type</p>
                                  <p className="text-sm text-foreground">{formatLabel(psc.kind)}</p>
                                </div>
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Notified</p>
                                  <p className="text-sm text-foreground">{formatDate(psc.notifiedOn)}</p>
                                </div>
                                <div>
                                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Ceased</p>
                                  <p className="text-sm text-foreground">—</p>
                                </div>
                              </div>
                            </article>
                      ...
                      <p className="text-sm text-muted-foreground">No current PSC entries returned in this snapshot.</p>
                    )}
                              ...
                              return (
                                ...
                                >
                                  <div className="grid gap-3 md:grid-cols-4 md:items-start">
                                    ...
                                    <div>
                                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Ceased</p>
                                      <p className="text-sm text-foreground">{formatDate(psc.ceasedOn)}</p>
                                    </div>
                                  </div>
                                </article>
                              );
                ...
                {companiesHouseTab === 'filings' && (
                  <div className="mt-6 space-y-3">
                    {companiesHouseProfile?.filingHistory?.items?.length ? (
                      <>
                        <div className="hidden rounded-xl border border-border bg-muted px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground md:grid md:grid-cols-[140px_120px_1fr]">
                          <span>Date</span>
                          <span>Doc Ref</span>
                          ...
                          >
                            <div className="grid gap-3 md:grid-cols-[140px_120px_1fr] md:items-start">
                              <div>
                                ...
                                <p className="text-sm text-foreground">{formatDate(filing.date)}</p>
                              </div>
                              <div>
                                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Doc Ref</p>
                                <p className="text-sm font-semibold text-foreground">{filing.type || '—'}</p>
                              </div>
                              <div>
                                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground md:hidden">Description</p>
                                <p className="font-semibold text-foreground">
                                  {formatCompaniesHouseFilingDescription(filing.description)}
                                </p>
                                <p className="mt-1 text-xs text-muted-foreground">{formatLabel(filing.category)}</p>
                              </div>
                            </div>
                          </article>
                        ))}
                      ...
                      <p className="text-sm text-muted-foreground">No filing history returned in current snapshot.</p>
                    )}
                  </div>
                )}
              </section>
                ...
                <div className="rounded-2xl border border-border bg-cardshadow-sm">
                  <header className="flex items-center justify-between">
                    <div>
                      ...
                      <h2 className="text-xl font-semibold text-foreground">Document Kits</h2>
                    </div>
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    {templateShortcuts.map((template) => (
                      <button
                        ...
                        className="w-full rounded-2xl border border-border/60 bg-muted text-left transition hover:bg-card"
                      >
              ...
              <section className="mt-6 rounded-2xl border border-border bg-cardshadow-sm">
                ...
                <div className="mt-4 space-y-3 text-sm">
                  {auditEvents.length === 0 ? (
                    <div className="rounded-xl border border-border/60 bg-muted text-muted-foreground">
                      No audit events recorded yet.
                    </div>
                  ) : (
                    auditEvents.map((event) => (
                    <article key={event.id} className="rounded-xl border border-border/60 bg-muted">
                      <div className="flex items-center justify-between">
                        <p className="font-semibold text-foreground">
      ...
      {contact && editingSection && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 p-4">
          <div className="mx-auto flex max-h-[calc(100vh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
            <div className="flex shrink-0 items-center justify-between border-b border-border px-6 py-4">
              <h3 className="text-xl font-semibold text-foreground">{SECTION_LABELS[editingSection]} Edit</h3>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateFieldStatusDraft(
                          'vat_number',
                          (event.target.value || 'missing') as FieldStatus
                        )
                      }
                    >
      ...
      {contact && isAddServiceOpen && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 p-4">
          <div className="mx-auto flex max-h-[calc(100vh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
            <div className="flex shrink-0 items-center justify-between border-b border-border px-6 py-4">
              <h3 className="text-xl font-semibold text-foreground">Add Service Engagement</h3>
              <button className="rounded-lg border border-border p-2 text-muted-foreground" onClick={() => setIsAddServiceOpen(false)}>
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-5">
              ...
              <div className="grid gap-3">
                <label className="text-sm font-semibold text-foreground">Service Template</label>
                <select
                ...
                <label className="text-sm font-semibold text-foreground">Service Fee</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                ...
                <label className="text-sm font-semibold text-foreground">{selectedServiceSchedule.inputLabel}</label>
                <input
                  type="date"
                  className="rounded-lg border border-border px-3 py-2 text-sm"
                ...
                />
                <p className="text-xs text-muted-foreground">{selectedServiceSchedule.helperText}</p>
              </div>
            </div>
            <div className="flex shrink-0 justify-end gap-2 border-t border-border px-6 py-4">
