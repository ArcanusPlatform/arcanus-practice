    ...
    <UniversalPageLayout>
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <nav
          ...
          className="mt-1 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground"
        >
          <button
          ...
          <span className="text-foreground">{companyNumber}</span>
        </nav>
        ...
        {loading ? (
          ...
          <>
            <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
              <header>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Import Setup</p>
                <h2 className="text-xl font-semibold text-foreground">Portfolio Selection</h2>
              </header>
            ...
            <section className="mt-6 rounded-3xl border border-border bg-card p-8 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h1 className="mt-2 text-3xl font-bold text-foreground">{company.companyName}</h1>
                  <p className="mt-1 text-sm text-muted-foreground">Company number {company.companyNumber}</p>
            ...
            <section className="mt-6 rounded-2xl border border-border bg-card p-2 shadow-sm">
              ...
              <>
                <section className="mt-6 grid gap-6 xl:grid-cols-2">
                  <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <header>
                      ...
                      <h2 className="text-xl font-semibold text-foreground">Registered Details</h2>
                    </header>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <header>
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Compliance Information</p>
                      <h2 className="text-xl font-semibold text-foreground">Accounts & Statements</h2>
                    </header>
                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                ...
                <section className="mt-6 grid gap-6 xl:grid-cols-2">
                  <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <header className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <h2 className="text-xl font-semibold text-foreground">Current Officers</h2>
                      </div>
                      <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold text-foreground inline-flex items-center gap-1">
                    ...
                    <div className="mt-4 space-y-4">
                      {currentOfficers.length === 0 ? (
                        ...
                        currentOfficers.map((officer, index) => (
                          ...
                          >
                            <h3 className="text-lg font-semibold text-foreground">{officer.displayName || officer.name}</h3>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <header className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Persons with Significant Control</p>
                        <h2 className="text-xl font-semibold text-foreground">Active PSC</h2>
                      </div>
                      <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold text-foreground inline-flex items-center gap-1">
                    ...
                    <div className="mt-4 space-y-4">
                      {currentPsc.length === 0 ? (
                        <p className="text-sm text-muted-foreground">No active PSC records found.</p>
                      ) : (
                        currentPsc.map((psc, index) => (
                          <article
                            ...
                            className="rounded-xl border border-border bg-muted p-4"
                          >
                            <h3 className="text-lg font-semibold text-foreground">{psc.displayName || psc.name}</h3>
                            <div className="mt-3 grid gap-3 md:grid-cols-2">
              ...
              <>
                <section className="mt-6 grid gap-6 xl:grid-cols-2">
                  <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <header>
                      ...
                      <h2 className="text-xl font-semibold text-foreground">Charges Summary</h2>
                    </header>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <header>
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Filing History</p>
                      <h2 className="text-xl font-semibold text-foreground">Filter by Category</h2>
                    </header>
                    <div className="mt-4 flex items-center gap-3">
                      <label htmlFor="filing-category" className="text-sm font-semibold text-foreground">
                        Category
                      </label>
                      <select
                        id="filing-category"
                ...
                <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
                  <header className="mb-4 flex items-center justify-between">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Filing History</p>
                      <h2 className="text-xl font-semibold text-foreground">Recent Filings</h2>
                    </div>
                    <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold text-foreground inline-flex items-center gap-1">
                  ...
                  {visibleFilingItems.length ? (
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          ...
                          <tbody>
                            {visibleFilingItems.map((item) => (
                              <tr key={`${item.transactionId}-${item.date}`} style={{ borderTop: '1px solid var(--color-border)' }}>
                    ...
                    <p className="text-sm text-muted-foreground">No filing history entries found for this category.</p>
                  )}
                </section>
              </>
            )}
            ...
            {activeTab === 'report' && (
              <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
                <header>
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Report Output</p>
                  <h2 className="text-xl font-semibold text-foreground">Companies House Comprehensive Report</h2>
